import { boxStyles } from '@src/constant/box_style.js';
...
const getBoxLayout = (message: string, options: LoggerBoxParameters) => {
  ...
  const style = boxStyles[options.boxStyle ?? 'rounded'];
...
const loggerBox = (message: string, options?: LoggerBoxParameters) => {
  if (isNode) {
